	...
	if not WORKINGURL == True:
		...
		if total == 1 and third == False:
			for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
				...
				return
		addFile('%s Version: %s' % (MCNAME, KODIV), '', icon=ICONBUILDS, themeit=THEME3)
		addFile('[COLOR dodgerblue]Version TVBAN Instalada:[/COLOR] %s %s' % (BUILDNAME, BUILDVERSION))              
		addDir ('Conservar Datos Importantes'       ,'savedata', icon=ICONSAVE,     themeit=THEME3)       
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
		if third == True:
			for item in addin:
		...
		if len(match) >= 1:
			if SEPERATE == 'true':
				...
				if count15 > 0:
					...
					addFile('[B]%s [COLOR red]Gracias por Instalar TVBAN (%s)[/COLOR][/B]' % (state, count15), 'togglesetting',  'show15', themeit=THEME2)
					if SHOW15 == 'false':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
...
def buildWizard(name, type, theme=None, over=False):
    ...
    if type == 'gui':
        ...
        if warning == True:
            yes_pressed = DIALOG.yesno("%s - [COLOR red]ATENCION[/COLOR]" % ADDONTITLE, '[COLOR %s]Vas a instalar una version Lite' % COLOR2, 'Quieres instalar: [COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR red]Cancelar[/COLOR][/B]',yeslabel='[B][COLOR dodgerblue]Instalar[/COLOR][/B]')
        else:
        ...
        if yes_pressed:
            ...
            if int(float(percent)) > 0:
                ...
                if int(float(errors)) > 0:
                    yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s v%s[/COLOR]' % (COLOR2, COLOR1, name, wiz.checkBuild(name,'version')), 'Completedo: [COLOR %s]%s%s[/COLOR] [Errores:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Quieres ver los errores?[/COLOR]', nolabel='[B][COLOR red]No[/COLOR][/B]', yeslabel='[B][COLOR dodgerblue]Si[/COLOR][/B]')
                    if yes:
